...
RSpec.describe Tappay::IPassMoney::Pay do
  let(:amount) { 1000 }
  let(:details) { 'Test Payment' }
  let(:merchant_id) { 'TEST_MERCHANT' }
  let(:prime) { 'ipass_money_prime' }
  let(:payment_url) { 'https://sandbox.tappaysdk.com/tpc/payment/pay-by-prime' }
  let(:frontend_redirect_url) { 'https://example.com/ipass_money/result' }
  let(:backend_notify_url) { 'https://example.com/ipass_money/notify' }
  let(:cardholder) do
  ...
  describe '#get_merchant_id' do
    let(:ipass_money_merchant_id) { 'IPASS_MONEY_MERCHANT' }

    context 'when ipass_money_merchant_id is configured' do
      before do
        allow(Tappay.configuration).to receive(:ipass_money_merchant_id).and_return(ipass_money_merchant_id)
      end

      it 'uses ipass_money_merchant_id' do
        expect(payment.send(:get_merchant_id)).to eq(ipass_money_merchant_id)
      end
    end

    context 'when ipass_money_merchant_id is not configured' do
      before do
        allow(Tappay.configuration).to receive(:ipass_money_merchant_id).and_return(nil)
        allow(Tappay.configuration).to receive(:merchant_group_id).and_return(nil)
    ...
    context 'when both merchant_group_id and ipass_money_merchant_id are configured' do
      before do
        allow(Tappay.configuration).to receive(:merchant_group_id).and_return('GROUP_ID')
        allow(Tappay.configuration).to receive(:ipass_money_merchant_id).and_return('IPASS_MONEY_MERCHANT')
      end
